...
#sys.path.append('/Library/python/2.7/site-packages/') #googlemaps')
...
stops = pd.read_csv(folder + 'google_transit_2/stops.txt',\
 sep=',',header=0)
 # Remove duplicates and AirBART "station"
stops = stops[stops['stop_id'] != "OAK"]
stops = stops[stops['stop_id'] != "19TH_N"]
stops = stops[stops['stop_id'] != "MCAR_S"]
...
sym = True
for st1 in stops['stop_id']:
    for st2 in neighbors[st1].keys():
        neighbors[st1][st2] = EuclideanDistance(st1, st2)

# Now create a matrix of distances between every pair of stations using
# Dijkstra's graph traversal algorithm
trips = pd.DataFrame({'Orig':['x'], 'Dest':['y'], 'EuclidMiles':[0], \
    'DrivingMiles':[0] })
for st1 in stops['stop_id']:
    for st2 in stops['stop_id']:
        trips = trips.append( {'Orig':st1, 'Dest':st2, \
        'EuclidMiles':max(Dijkstra(neighbors, st1, st2)[0].values()),\
        'DrivingMiles': 0 }, True)
# Remove firt row, which was justfor initialization
trips = trips[1:] # indexing starts at zero
if(writeYN):
    trips.to_csv('/volumes/thumbie/BART project/data/BART/Euclidean miles.csv', \
    index=False)